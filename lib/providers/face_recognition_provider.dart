...
class FaceRecognitionProvider with ChangeNotifier {
  ...
  Future<bool> checkStudentImageExists(String studentId) async {
    try {
      ...
      final exists = await _faceRecognitionService.checkStudentImageExists(studentId);
  ...
  Future<String?> uploadStudentImage(ImageSource source, {XFile? pickedFile}) async {
    try {
      _isLoading = true;
      _error = null;
      _uploadProgress = 0.0;
